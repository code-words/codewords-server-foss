...
describe GameDataChannel, type: :channel do
  ...
  it 'advances the game if player ends turn early' do
    spy = @game.players.create(user: User.create(name: "Cheryl"), role: :spy, team: :red)
    stub_connection current_player: spy
    subscription = subscribe
    ...
    expect{subscription.send_guess({"id" => nil})}
      .to have_broadcasted_to(@game)
      .from_channel(GameDataChannel)
      .once
      .with{ |data|
        ...
        payload = message[:data]
        received_card = payload[:card]
        expect(received_card).to eq(nil)
  ...
  it 'advances the game if no guesses remaining' do
    spy = @game.players.create(user: User.create(name: "Cheryl"), role: :spy, team: :red)
    stub_connection current_player: spy
    subscription = subscribe
  ...
  it 'ends the game if the guess was for the last of player\'s cards' do
    ...
    expect{subscription.send_guess({"id" => "#{guess_card.id}"})}
      ...
      .with{ |data|
        ...
        expect(payload[:winningTeam]).to eq(built_player.team)
      }
  end

  it 'ends the game if the guess was for the assassin card' do
    spy = @game.players.create(user: User.create(name: "Cheryl"), role: :spy, team: :red)
    stub_connection current_player: spy
    subscription = subscribe
